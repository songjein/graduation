...
from flask import render_template, request, redirect, url_for, flash, session, g, jsonify, send_from_directory
...
def main():
    ...
    for i in range(len(fbfriends)):
     	if User.query.get(fbfriends[i]['id']) != None:
...
def create_project():
	...
	#return str(proj.id)
        ...
        if request.files['photo']:
	        ...
	        file_key = str(proj.id) + "." +  post_data.filename.rsplit('.',1)[1]
		# file save
                post_data.save(os.path.join(app.root_path, app.config['UPLOAD_FOLDER'], file_key))
		# file name save
	       	proj.file_key = file_key
	    else:
	        return "허용된 파일 형식이 아닙니다."   
	else:
	    return "사진을 반드시 등록해주세요"
...
def make_log(project_id):
    ...
    if request.method == 'POST':
	    ...
	    file_key = str(upload_data.key()) # file name
        ...
        tmp = ""
        if project.tags_list == None or len(project.tags_list) == 0:
            flag = False
        ...
        if 'fbcheck' in request.form and request.form['fbcheck'] != None and request.form['fbcheck'] == "on" :
            userinfo = facebook.post('/me/feed', 
                data={
                'link': 'http://study4share.xyz/project_detail/'+ project_id, 
...
def shows(key):
    ...
    return send_from_directory(os.path.join(app.root_path, app.config['UPLOAD_FOLDER']), key)
